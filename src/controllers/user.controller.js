...
const createUser = async (req, res) => {
    try {
        ...
        .then((status) => {
            if (!status) console.log("⚠️ Unable send the verification email to:", email)
        })
...
const updateUser = async (req, res) => {
    try {
        ...
        const updatedUser = await userModel.updateUser(userId, fullName, email, hashedPassword)
    ...
    try {
        ...
        if (!foundUser.is_verified) {
            return errorHandler(
                ...
                false,
                403,
                "Account not verified. Please check your email to complete the verification."
            )
        }
